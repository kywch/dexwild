...
The XC430s do not have current mode so we must rely on pwm control mode for this and then set pwm limits instead of current limits at runtime.
...
def main(fn, isLeftcmd):
    ...
    ma_high = signed_to_unsigned(isLeft * (-200/2.69), 2)  #ma * 2.69
    ma_high_neg = signed_to_unsigned(isLeft * (200/2.69),  2)  #ma * 2.69
